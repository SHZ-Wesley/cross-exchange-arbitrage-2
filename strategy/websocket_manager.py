"""WebSocket management for Maker (Generic) and Lighter exchanges."""
import asyncio
import json
import logging
import time
...
class WebSocketManagerWrapper:
    ...
    def set_maker_ws_config(self, client: BaseExchangeClient, contract_id: str):
        """Set Maker WebSocket client config."""
    ...
    # Maker WebSocket methods
    def handle_maker_order_book_update(self, message):
        """Handle Maker order book updates from WebSocket."""
        # Note: This is specifically for EdgeX format if using WebSocketManager directly.
        # For generalized solution, clients should handle their own WS and update OrderBookManager directly.
        # However, keeping this for backward compat if EdgeX raw messages are passed here.
        try:
            if isinstance(message, str):
                message = json.loads(message)

            self.logger.debug(f"Received Maker depth message: {message}")

            # Check if this is a quote-event message with depth data (EdgeX specific pattern)
                ...
                if channel.startswith("depth."):
                    ...
                    if data and len(data) > 0:
                        ...
                        if depth_type in ['SNAPSHOT', 'CHANGED']:
                            ...
                            self.order_book_manager.update_maker_order_book(bids, asks)

        except Exception as e:
            self.logger.error(f"Error handling Maker order book update: {e}")
    ...
    def shutdown(self):
        ...
        # Note: Maker WS is generally managed by the Maker client itself in the new architecture.
        # This wrapper is mostly for Lighter WS now.
