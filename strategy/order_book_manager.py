"""Order book management for Maker (EdgeX/Paradex/GRVT) and Lighter exchanges."""
import asyncio
import logging
...
class OrderBookManager:
    ...
    def __init__(self, logger: logging.Logger):
        ...
        # Maker (EdgeX/Paradex/GRVT) order book state
        self.maker_order_book = {'bids': {}, 'asks': {}}
        self.maker_best_bid: Optional[Decimal] = None
        self.maker_best_ask: Optional[Decimal] = None
        self.maker_order_book_ready = False
    ...
    # Maker order book methods
    def update_maker_order_book(self, bids: list, asks: list):
        """Update Maker order book with new levels."""
        # Update bids
        for bid in bids:
            ...
            elif isinstance(bid, list) or isinstance(bid, tuple):
                price = Decimal(bid[0])
                size = Decimal(bid[1])
            else:
                continue
            ...
            if size > 0:
                self.maker_order_book['bids'][price] = size
            else:
                self.maker_order_book['bids'].pop(price, None)
        ...
        for ask in asks:
            ...
            elif isinstance(ask, list) or isinstance(ask, tuple):
                price = Decimal(ask[0])
                size = Decimal(ask[1])
            else:
                continue
            ...
            if size > 0:
                self.maker_order_book['asks'][price] = size
            else:
                self.maker_order_book['asks'].pop(price, None)

        # Update best bid and ask
        if self.maker_order_book['bids']:
            self.maker_best_bid = max(self.maker_order_book['bids'].keys())
        if self.maker_order_book['asks']:
            self.maker_best_ask = min(self.maker_order_book['asks'].keys())

        if not self.maker_order_book_ready:
            self.maker_order_book_ready = True
            self.logger.info(f"📊 Maker order book ready - Best bid: {self.maker_best_bid}, "
                             f"Best ask: {self.maker_best_ask}")
        else:
            self.logger.debug(f"📊 Order book updated - Best bid: {self.maker_best_bid}, "
                              f"Best ask: {self.maker_best_ask}")

    def get_maker_bbo(self) -> Tuple[Optional[Decimal], Optional[Decimal]]:
        """Get Maker best bid/ask prices."""
        return self.maker_best_bid, self.maker_best_ask
